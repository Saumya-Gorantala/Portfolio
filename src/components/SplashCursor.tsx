...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    /**
     * generateColor — constrained to red/pink/rose palette
     * Hue range: 330°–360° (pink-red) + 0°–15° (red-orange-red)
     * mapped from HSV hue in [0,1] → narrow band
     */
    function generateColor() {
      // Pick a hue biased toward reds and pinks
      // HSV hue: 0=red, 0.917=magenta-pink, 0.95=hot-pink
      const hueChoices = [
        0.0,   // pure red
        0.02,  // red-orange side (still red)
        0.92,  // hot pink
        0.95,  // deep pink / rose
        0.97,  // crimson-pink
        1.0,   // cycles back to red
      ];
      const h = hueChoices[Math.floor(Math.random() * hueChoices.length)]
        + (Math.random() - 0.5) * 0.03; // tiny jitter

      const s = 0.85 + Math.random() * 0.15; // high saturation
      const v = 0.9 + Math.random() * 0.1;   // bright

      let c = HSVtoRGB(((h % 1) + 1) % 1, s, v);
      // Scale down for fluid (same as original * 0.15)
      c.r *= 0.15;
      c.g *= 0.15;
      c.b *= 0.15;
      return c;
